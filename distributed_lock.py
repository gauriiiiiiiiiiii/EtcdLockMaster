...
def safe_revoke(lease):
    """Revoke a lease, ignoring already-expired leases."""
    try:
        lease.revoke()
...
def _wait_for_deletion(key: str, wait_time) -> bool:
    """Block until key is deleted or wait_time seconds pass. None = wait forever."""
    ...
    events, cancel = client.watch(key)  # watch starts before existence check (race-safe)
...
def acquire_lock(resource: str, ttl: int = 30, timeout: float = None):
    """Acquire a distributed lock. Returns (owner_id, lease) or raises TimeoutError."""
...
class DistributedLock:
    """Re-entrant distributed lock context manager backed by etcd."""
    ...
    def __init__(self, resource: str, ttl: int = 30, timeout: float = None, auto_renew: bool = False):
        ...
        self._count = 0  # re-entrancy depth counter

    def __enter__(self):
        if self._count > 0:  # already held by this instance
            self._count += 1
        ...
        if self.auto_renew:
            ...
            def _renew_loop():
                ...
                while not self._stop_event.wait(interval):  # wait() wakes instantly on set()
                    try:
                        self.lease.refresh()
    ...
    def is_locked(self) -> bool:
        """Live etcd check — True if this instance still holds the key."""
